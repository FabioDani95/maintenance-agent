...
In `search_past_events`, chat questions may include natural date scopes. The router converts supported phrases into the same structured `date_from` / `date_to` filters used by the log APIs, then removes the temporal wording from the semantic retrieval query. Supported examples include `past 2 months`, `last 3 months`, `past year` (rolling year), `last year` / `previous year` (previous calendar year), `from January 10 to March 4`, `from 10 January to 4 March`, and `dal 10 gennaio al 4 marzo`. Date-only `date_to` remains exclusive, so an end date mentioned by the user is sent as the following day.
